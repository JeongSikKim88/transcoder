...
import (
	...
	"github.com/org_transcoder/transcoder"
	"github.com/org_transcoder/transcoder/utils"
)
...
func (t *Transcoder) Start(opts transcoder.Options) (<-chan transcoder.Progress, error) {
	...
	var args []string
	if t.inputImage == "" {
		args = append([]string{"-i", t.input, "-hide_banner"}, opts.GetStrArguments()...)
		// args = append([]string{"-i", t.input, "-threads", "0"}, opts.GetStrArguments()...)
	} else {
		args = append([]string{"-i", t.input, "-i", t.inputImage, "-hide_banner"}, opts.GetStrArguments()...)
	}
	...
	if t.config.ProgressEnabled && !t.config.Verbose {
		...
		if err != nil {
			ErrorLogger.Println("Failed getting transcoding progress", t.config.FfmpegBinPath, "with args", args, "with error", err)
	...
	err = cmd.Start()
	if err != nil {
		ErrorLogger.Println("Failed starting transcoding", t.config.FfmpegBinPath, "with args", args, "with error", err)
...
func (t *Transcoder) getMetadata() (metadata *Metadata, err error) {
	...
	if t.config.FfprobeBinPath != "" {
		...
		args := []string{"-i", input, "-hide_banner", "-show_format", "-show_streams", "-show_error"}
		...
		err := cmd.Run()
		if err != nil {
			ErrorLogger.Println("error executing", t.config.FfprobeBinPath, "| error:", err, "| message:", errb.String())
			defer func() {
				s := recover()
				fmt.Println(s)
			}()
			panic(err)
			return nil, err
			// return nil, fmt.Errorf("error executing", t.config.FfprobeBinPath, "| error:", err, "| message:", errb.String())
			// return nil, fmt.Println("error executing (%s) with args (%s) | error: %s | message: %s %s", t.config.FfprobeBinPath, args, err, outb.String(), errb.String())
		}
